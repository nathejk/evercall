...
name: prd
description: >
  Author and maintain Product Requirements Documents (PRDs) in the evercall
...
1. **Gather context first.** Before drafting, make sure you understand:
   - The problem being solved and who has it. Note that this service has **no
     end users**: its consumers are other systems (hq and anything else
     subscribing to the call events), so state which consumer needs what.
   - Any related existing code in `go/` — `cmd/evercall` (transport) and
     `internal/` (ingest, fetcher, evercall client, recordings).
   - Whether the feature touches the webhook ingest path, the recording
     fetcher, the published event contract, or several.
...
3. **Respect repo conventions:**
   - This is a headless integration service: webhook in, JetStream events out,
     recordings fetched asynchronously. There is no frontend and no read model;
     a PRD that assumes either must justify introducing it explicitly.
   - **Any change to a published event subject or payload is a cross-repo
     contract change.** Say so in Technical Considerations, name the consumers
     (hq, …), and note that the payload type lives in `shared-go` and must be
     released there first.
   - Call out webhook-specific concerns where relevant: sender verification,
     idempotency on redelivery, and what the endpoint returns on failure.
